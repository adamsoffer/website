...
/**
 * Developer Portal header — full-width app bar, zero props.
          ...
          >
            ...
            <div className="py-1.5 px-1.5">
              {[
                ...
                { label: "API Tokens", href: "/portal/settings?tab=tokens", icon: Key },
              ].map((item) => (
                <Link
                  key={item.label}
                  href={item.href}
...
function Breadcrumb({ pathname }: { pathname: string }) {
  ...
  // Special case: /portal/models/[id] → Explore / <Model name>
  ...
  if (segments[0] === "models" && segments[1]) {
    ...
    crumbs = [
      { href: "/portal/explore", label: "Explore" },
      {
        href: `/portal/models/${segments[1]}`,
    ...
    crumbs = segments.map((segment, i) => ({
      href: "/portal/" + segments.slice(0, i + 1).join("/"),
  ...
  return (
    <div className="flex items-center">
      ...
      {/* Crumb 2: Developer Portal green pill → /portal */}
      <Link href="/portal">
        <Badge variant="app">Developer Portal</Badge>
      </Link>
...
function getTabActive(itemHref: string, pathname: string): boolean {
  ...
  if (itemHref === "/portal/explore")
    return (
      pathname === "/portal/explore" ||
      pathname.startsWith("/portal/explore/") ||
      pathname.startsWith("/portal/models/")
    );
  ...
  return (
    <>
      ...
      <PortalMobileDrawer
        ...
        onSignIn={() => router.push("/portal/login")}
      />
