...
import { generateCardBackground } from "@/lib/portal/generate-card-visual";
            ...
            <li className="-mx-3 flex items-center gap-3 rounded-lg px-3 py-2.5 opacity-60">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs text-white/55">
                  Welcome to the Livepeer Developer Portal.
                </p>
              </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {HOME_CATEGORIES.map(({ label, subtitle }, i) => {
          ...
          return (
            <Link
              key={label}
              href={`/portal/explore?category=${encodeURIComponent(label)}`}
      ...
      <div className="overflow-hidden rounded-xl border border-white/[0.06] bg-dark-surface">
        {rows.map((row) => {
          ...
          return (
            <Link
              key={row.id}
              href={`/portal/settings?tab=usage&request=${row.id}`}
